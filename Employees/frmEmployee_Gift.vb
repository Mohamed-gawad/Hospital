...
Public Class frmEmployee_Gift
    ...
    Sub Fillgrd() ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''لتعبئة الداتا جريد بالبيانات 
        drg.Rows.Clear()
        Myconn.Filldataset("select e.EmployeeName,j.jobname,s.Employee_Salary,* from Employees_Gift a
    ...
    Sub Binding() '''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''لتعبئة مربعات النصوص بالبيانات 
        ...
        Dim Myfields() As String = {"Employee_Salary", "Employee_Gift", "Notes"}
    ...
    Sub Save_Recod()
        Try
            Dim sql As String = "INSERT INTO Employees_Gift(EmployeeID,Day_Date,Employee_Gift,Notes) VALUES(@EmployeeID,@Day_Date,@Employee_Gift,@Notes)"
            ...
            With Myconn.cmd.Parameters
                ...
                .AddWithValue("@Employee_Gift", txtHafes.Text)
    ...
    End Sub
    Private Sub frmEmployee_Gift_Load(sender As Object, e As EventArgs) Handles Me.Load
        Label23.Left = 0
    ...
    Private Sub btnDel_Click(sender As Object, e As EventArgs) Handles btnDel.Click
        ...
        Else
            ...
            Myconn.DeleteRecord("Employees_Gift", "ID", CInt(drg.CurrentRow.Cells(8).Value))
    ...
    Private Sub btnUpdat_Click(sender As Object, e As EventArgs) Handles btnUpdat.Click
        ...
        Next
        Try
            Dim sql As String = "Update  Employees_Gift set EmployeeID=@EmployeeID,Day_Date=@Day_Date,Employee_Gift=@Employee_Gift,Notes=@Notes where ID=@ID"
            Myconn.cmd = New SqlCommand(sql, Myconn.conn)
            With Myconn.cmd.Parameters
                .AddWithValue("@EmployeeID", cbo_Employee.SelectedValue)
                .AddWithValue("@Day_Date", Format(CDate(txtDate.Text), "yyyy/MM/dd"))
        ...
        Myconn.Filldataset("select e.EmployeeName,j.jobname,s.Employee_Salary,* from Employees_Gift a
                           left join Employees_Salary s on a.EmployeeID = s.EmployeeID   
                           left join Employees e on a.EmployeeID = e.EmployeeID 
                           left join Jobs j on e.jobID = j.jobID where a.ID =" & CInt(drg.CurrentRow.Cells(9).Value), "Employees_Gift", Me)
        ...
        drg.CurrentRow.Cells(6).Value = Myconn.cur.Current("Employee_Gift")
