Public Class frmPharm_Kinds_Move
    ...
    Sub Fillgrd()
        Try
            Select Case x
                Case 0
                    Myconn.Filldataset("select g.GroupName,d.Max_Unit_Name,e.Min_Unit_Name,a.Min_Unit_number,a.Drug_Name,a.Drug_ID,a.Drug_Price,isnull(b.Pucr_amount,0) as Pucr_amount,isnull((isnull(c.Amount_max,0) * a.Min_Unit_number + isnull(c.Amount_min,0)),0) as sales_amount,
                           ((isnull(b.Pucr_amount,0) * a.Min_Unit_number) - isnull((isnull(c.Amount_max,0) * a.Min_Unit_number + isnull(c.Amount_min,0)),0)) as rest from [dbo].[Drugs] a
                            left join (select Drug_ID,sum(Drug_Amount + Drug_Bonus) as Pucr_amount from [dbo].[Drug_Purchases] group by Drug_ID,state having state = 'True') b
                            ...
                            left join (select Drug_ID,sum(Amount_max) as Amount_max,sum(Amount_min) as Amount_min from [dbo].[Drug_Sales] group by Drug_ID ,state having state = 'True') c
                            on a.Drug_ID=c.Drug_ID", "Drug_Purchases", Me)
                Case 1
                    Myconn.Filldataset("select g.GroupName,d.Max_Unit_Name,e.Min_Unit_Name,a.Min_Unit_number,a.Drug_Name,a.Drug_ID,a.Drug_Price,isnull(b.Pucr_amount,0) as Pucr_amount,isnull((isnull(c.Amount_max,0) * a.Min_Unit_number + isnull(c.Amount_min,0)),0) as sales_amount,
                           ((isnull(b.Pucr_amount,0) * a.Min_Unit_number) - isnull((isnull(c.Amount_max,0) * a.Min_Unit_number + isnull(c.Amount_min,0)),0)) as rest from [dbo].[Drugs] a
                            left join (select Drug_ID,sum(Drug_Amount + Drug_Bonus) as Pucr_amount from [dbo].[Drug_Purchases] group by Drug_ID,state having state = 'True') b
                            ...
                            left join Min_Unit e on a.Min_UnitID = e.Min_UnitID
                            left join (select Drug_ID,sum(Amount_max) as Amount_max,sum(Amount_min) as Amount_min from [dbo].[Drug_Sales] group by Drug_ID ,state having state = 'True') c
                            on a.Drug_ID=c.Drug_ID where a.GroupID =" & CInt(cbo_Group.ComboBox.SelectedValue), "Drug_Purchases", Me)
            End Select
            ...
            For i As Integer = 0 To Myconn.cur.Count - 1
                ...
                drg.Rows(i).Cells(4).Value = Myconn.cur.Current("Pucr_amount") & Space(1) & Myconn.cur.Current("Max_Unit_Name")
                drg.Rows(i).Cells(5).Value = If(Myconn.cur.Current("sales_amount") Mod Myconn.cur.Current("Min_Unit_number") = 0, Math.Truncate(Myconn.cur.Current("sales_amount") / Myconn.cur.Current("Min_Unit_number")) & Space(1) & Myconn.cur.Current("Max_Unit_Name"), Math.Truncate(Myconn.cur.Current("sales_amount") / Myconn.cur.Current("Min_Unit_number")) & Space(1) & Myconn.cur.Current("Max_Unit_Name") & " و " & Myconn.cur.Current("sales_amount") Mod Myconn.cur.Current("Min_Unit_number") & Space(1) & Myconn.cur.Current("Min_Unit_Name"))
                drg.Rows(i).Cells(6).Value = If(Myconn.cur.Current("rest") Mod Myconn.cur.Current("Min_Unit_number") = 0, Math.Truncate(Myconn.cur.Current("rest") / Myconn.cur.Current("Min_Unit_number")) & Space(1) & Myconn.cur.Current("Max_Unit_Name"), Math.Truncate(Myconn.cur.Current("rest") / Myconn.cur.Current("Min_Unit_number")) & Space(1) & Myconn.cur.Current("Max_Unit_Name") & " و " & Myconn.cur.Current("rest") Mod Myconn.cur.Current("Min_Unit_number") & Space(1) & Myconn.cur.Current("Min_Unit_Name"))
                drg.Rows(i).Cells(7).Value = Val(Myconn.cur.Current("rest") * Myconn.cur.Current("Drug_Price")) / Val(Myconn.cur.Current("Min_Unit_number"))
                drg.Rows(i).Cells(8).Value = Myconn.cur.Current("GroupName")
                Myconn.cur.Position += 1
            Next
            Myconn.Sum_drg(drg, 7, Label2, Label1)
        Catch ex As Exception
            MsgBox("هناك خطأ ما")
            Return
        End Try
    End Sub
    Private Sub frmPharm_Kinds_Move_Load(sender As Object, e As EventArgs) Handles Me.Load
        Label3.Left = 0
