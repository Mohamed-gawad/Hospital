...
Public Class frmPharm_safe_Chart
            ...
            Select Case y
                Case 0
                    If cbo_Ezn.SelectedIndex = 0 Then
                        Myconn.Filldataset("Select state,(cast(DATEPART(yyyy,P_Date) as varchar(4)) + '/' + cast(format(DATEPART(MM,P_Date),'00') as varchar(2))) as Payment_Month,
                                        sum(amount) as amount from Pharm_Safe_Payment group by (cast(DATEPART(yyyy,P_Date) as varchar(4)) + '/' + cast(format(DATEPART(MM,P_Date),'00') as varchar(2))),state
                                         having state ='True'" & st, "Pharm_Safe_Payment", Me)
                    ...
                    ElseIf cbo_Ezn.SelectedIndex = 1 Then
                        ...
                        Myconn.Filldataset("Select state,(cast(DATEPART(yyyy,P_Date) as varchar(4)) + '/' + cast(format(DATEPART(MM,P_Date),'00') as varchar(2))) as recive_Month,
                                        sum(amount) as amount from Pharm_Safe_recive group by (cast(DATEPART(yyyy,P_Date) as varchar(4)) + '/' + cast(format(DATEPART(MM,P_Date),'00') as varchar(2))),state
                                         having state ='True'" & st, "Pharm_Safe_recive", Me)
                    ...
                    ElseIf cbo_Ezn.SelectedIndex = 2 Then
                        ...
                        Myconn.Filldataset2("Select state,(cast(DATEPART(yyyy,P_Date) as varchar(4)) + '/' + cast(format(DATEPART(MM,P_Date),'00') as varchar(2))) as Payment_Month,
                                        sum(amount) as amount from Pharm_Safe_Payment group by (cast(DATEPART(yyyy,P_Date) as varchar(4)) + '/' + cast(format(DATEPART(MM,P_Date),'00') as varchar(2))),state
                                         having state ='True'" & st, "Pharm_Safe_Payment", Me)
                ...
                Case 1 ' البند
                    If cbo_Ezn.SelectedIndex = 0 Then
                        Myconn.Filldataset("Select state,(cast(DATEPART(yyyy,P_Date) as varchar(4)) + '/' + cast(format(DATEPART(MM,P_Date),'00') as varchar(2))) as Payment_Month,
                                        sum(amount) as amount from Pharm_Safe_Payment group by (cast(DATEPART(yyyy,P_Date) as varchar(4)) + '/' + cast(format(DATEPART(MM,P_Date),'00') as varchar(2))),state,itemID
                                         having state ='True' and itemID =" & CInt(cbo_Band.ComboBox.SelectedValue) & st, "Pharm_Safe_Payment", Me)
                    ...
                    ElseIf cbo_Ezn.SelectedIndex = 1 Then
                        ...
                        Myconn.Filldataset("Select state,(cast(DATEPART(yyyy,P_Date) as varchar(4)) + '/' + cast(format(DATEPART(MM,P_Date),'00') as varchar(2))) as recive_Month,
                                        sum(amount) as amount from Pharm_Safe_recive group by (cast(DATEPART(yyyy,P_Date) as varchar(4)) + '/' + cast(format(DATEPART(MM,P_Date),'00') as varchar(2))),state,itemID
                                         having state ='True' and itemID =" & CInt(cbo_Band.ComboBox.SelectedValue) & st, "Pharm_Safe_recive", Me)
                ...
                Case 2 ' العميل أو المورد
                    If cbo_Ezn.SelectedIndex = 0 Then
                        Myconn.Filldataset("Select state,(cast(DATEPART(yyyy,P_Date) as varchar(4)) + '/' + cast(format(DATEPART(MM,P_Date),'00') as varchar(2))) as Payment_Month,
                                        sum(amount) as amount from Pharm_Safe_Payment group by (cast(DATEPART(yyyy,P_Date) as varchar(4)) + '/' + cast(format(DATEPART(MM,P_Date),'00') as varchar(2))),state,Supplier_ID
                                         having state ='True' and Supplier_ID =" & CInt(cbo_Band.ComboBox.SelectedValue) & st, "Pharm_Safe_Payment", Me)
                    ...
                    ElseIf cbo_Ezn.SelectedIndex = 1 Then
                        ...
                        Myconn.Filldataset("Select state,(cast(DATEPART(yyyy,P_Date) as varchar(4)) + '/' + cast(format(DATEPART(MM,P_Date),'00') as varchar(2))) as recive_Month,
                                        sum(amount) as amount from Pharm_Safe_recive group by (cast(DATEPART(yyyy,P_Date) as varchar(4)) + '/' + cast(format(DATEPART(MM,P_Date),'00') as varchar(2))),state,Customer_ID
                                         having state ='True' and Customer_ID =" & CInt(cbo_Band.ComboBox.SelectedValue) & st, "Pharm_Safe_recive", Me)
    ...
    Private Sub frmPharm_safe_Chart_Load(sender As Object, e As EventArgs) Handles Me.Load
        Label3.Left = 0
    ...
    Private Sub cbo_Search_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cbo_Search.SelectedIndexChanged
        ...
        Select Case cbo_Search.SelectedIndex
            ...
            Case 2 ' العميل
                ...
                If cbo_Ezn.SelectedIndex = 1 Then
                    cbo_Band.Visible = True
                    fin = False
                    Myconn.Fillcombo("select * from Customers order by Customer_Name", "Customers", "Customer_ID", "Customer_Name", Me, cbo_Band.ComboBox)
                    fin = True
                End If
                y = 2
            Case 3 ' المورد
        ...
        If txt1.Text.Length >= 7 And txt2.Text.Length >= 7 Then
            Select Case cbo_Search.SelectedIndex
                ...
                Case 3
                    y = 2
            End Select
            st = "and (cast(DATEPART(yyyy,P_Date) as varchar(4)) + '/' + cast(format(DATEPART(MM,P_Date),'00') as varchar(2))) between '" & Format(CDate(txt1.Text), "yyyy/MM") & "' and '" & Format(CDate(txt2.Text), "yyyy/MM") & "'"
            Chart_Sfe_Move()
        ...
        ElseIf txt1.Text.Length = 4 And txt2.Text.Length = 4 Then
            ...
            Select Case cbo_Search.SelectedIndex
                    ...
                    Case 3
                        y = 2
                End Select
            st = "and cast(DATEPART(yyyy,P_Date) as varchar(4))  between '" & txt1.Text & "' and '" & txt2.Text & "'"
            Chart_Sfe_Move_Year()
        End If
    ...
    Sub Chart_Sfe_Move_Year()
        ...
        Try
            Select Case y
                Case 0
                    If cbo_Ezn.SelectedIndex = 0 Then
                        Myconn.Filldataset("Select state,cast(DATEPART(yyyy,P_Date) as varchar(4)) as Payment_Year,
                                        sum(amount) as amount from Pharm_Safe_Payment group by cast(DATEPART(yyyy,P_Date) as varchar(4)),state
                                         having state ='True'" & st, "Pharm_Safe_Payment", Me)
                    ...
                    ElseIf cbo_Ezn.SelectedIndex = 1 Then
                        ...
                        Myconn.Filldataset("Select state,cast(DATEPART(yyyy,P_Date) as varchar(4)) as recive_Year,
                                        sum(amount) as amount from Pharm_Safe_recive group by cast(DATEPART(yyyy,P_Date) as varchar(4)),state
                                         having state ='True'" & st, "Pharm_Safe_recive", Me)
                    ...
                    ElseIf cbo_Ezn.SelectedIndex = 2 Then
                        ...
                        Myconn.Filldataset2("Select state,cast(DATEPART(yyyy,P_Date) as varchar(4)) as Payment_Year,
                                        sum(amount) as amount from Pharm_Safe_Payment group by cast(DATEPART(yyyy,P_Date) as varchar(4)),state
                                         having state ='True'" & st, "Pharm_Safe_Payment", Me)
                ...
                Case 1 ' البند
                    If cbo_Ezn.SelectedIndex = 0 Then
                        Myconn.Filldataset("Select state,cast(DATEPART(yyyy,P_Date) as varchar(4)) as Payment_Year,
                                        sum(amount) as amount from Pharm_Safe_Payment group by cast(DATEPART(yyyy,P_Date) as varchar(4)),state,itemID
                                         having state ='True' and itemID =" & CInt(cbo_Band.ComboBox.SelectedValue) & st, "Pharm_Safe_Payment", Me)
                    ...
                    ElseIf cbo_Ezn.SelectedIndex = 1 Then
                        ...
                        Myconn.Filldataset("Select state,cast(DATEPART(yyyy,P_Date) as varchar(4)) as recive_Year,
                                        sum(amount) as amount from Pharm_Safe_recive group by cast(DATEPART(yyyy,P_Date) as varchar(4)),state,itemID
                                         having state ='True' and itemID =" & CInt(cbo_Band.ComboBox.SelectedValue) & st, "Pharm_Safe_recive", Me)
                ...
                Case 2 ' العميل أو المورد
                    If cbo_Ezn.SelectedIndex = 0 Then
                        Myconn.Filldataset("Select state,cast(DATEPART(yyyy,P_Date) as varchar(4)) as Payment_Year,
                                        sum(amount) as amount from Pharm_Safe_Payment group by cast(DATEPART(yyyy,P_Date) as varchar(4)),state,Supplier_ID
                                         having state ='True' and Supplier_ID =" & CInt(cbo_Band.ComboBox.SelectedValue) & st, "Pharm_Safe_Payment", Me)
                    ...
                    ElseIf cbo_Ezn.SelectedIndex = 1 Then
                        ...
                        Myconn.Filldataset("Select state,cast(DATEPART(yyyy,P_Date) as varchar(4)) as recive_Year,
                                        sum(amount) as amount from Pharm_Safe_recive group by cast(DATEPART(yyyy,P_Date) as varchar(4)),state,Customer_ID
                                         having state ='True' and Customer_ID =" & CInt(cbo_Band.ComboBox.SelectedValue) & st, "Pharm_Safe_recive", Me)
