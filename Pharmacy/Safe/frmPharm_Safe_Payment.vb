...
Public Class frmPharm_Safe_Payment
    Dim Myconn As New connect
    Dim x As Integer
    Sub NewRecord()                                           '''''''''''''''''''''''''''لعمل سجل جديد وإعطائه رقم
        Myconn.Autonumber("P_ID", "Pharm_Safe_Payment", txtID, Me)
    ...
    Sub Fillgrd() '''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''لتعبئة الداتا جريد بالبيانات 
        ...
        Select Case x
            Case 0
                Myconn.Filldataset("Select * ,b.itemName,c.Supplier_Name,d.EmployeeName from Pharm_Safe_Payment a
                           ...
                           left join Supplier c on a.Supplier_ID = c.Supplier_ID 
                            left join Employees d on a.User_ID = d.EmployeeID", "Pharm_Safe_Payment", Me)
            ...
            Case 2
                ...
                drg.CurrentRow.Cells(3).Value = Myconn.cur.Current("Supplier_Name")
    ...
    End Sub
    Sub SaveKind()
        Dim sql As String = "INSERT INTO Pharm_Safe_Payment(P_ID,P_Date,itemID,amount,amount_abc,Note,State,PermissionID,Supplier_ID,User_ID) 
                            VALUES(@P_ID,@P_Date,@itemID,@amount,@amount_abc,@Note,@State,@PermissionID,@Supplier_ID,@User_ID)"
        ...
        With Myconn.cmd.Parameters
            ...
            .Add("@PermissionID", SqlDbType.TinyInt).Value = 2
            .Add("@Supplier_ID", SqlDbType.Int).Value = If(cbo_Supplier.SelectedIndex = -1, DBNull.Value, cbo_Supplier.SelectedValue)
        ...
        Myconn.DataGridview_MoveLast(drg, 2)
    End Sub
    Private Sub frmPharm_Safe_Payment_Load(sender As Object, e As EventArgs) Handles Me.Load
        Label5.Left = 0
        Label5.Width = Me.Width
        Me.KeyPreview = True

        Myconn.Fillcombo("select * from payment_item order by itemName", "payment_item", "paymentID", "itemName", Me, cbo_band)
    ...
    Private Sub btnSave_Click(sender As Object, e As EventArgs) Handles btnSave.Click
        For Each txt As Control In GroupBox1.Controls
            ...
            ElseIf TypeOf txt Is ComboBox Then
                If txt.Text = "" And txt.Name IsNot "cbo_Supplier" Then
                    ErrorProvider1.SetError(txt, "أكمل البيانات")
                    MessageBox.Show("من فضلك أكمل البيانات ", "رسالة تحذير", MessageBoxButtons.OK, MessageBoxIcon.Warning, MessageBoxDefaultButton.Button1, MessageBoxOptions.RtlReading)
                    Return
    ...
    Private Sub btnDel_Click(sender As Object, e As EventArgs) Handles btnDel.Click
        ...
        If (result = DialogResult.No) Then
            Return
        Else
            Myconn.DeleteRecord("Pharm_Safe_Payment", "ID", CInt(drg.CurrentRow.Cells(6).Value))
            x = 0
            Fillgrd()
        End If
    End Sub
    Private Sub btnUpdat_Click(sender As Object, e As EventArgs) Handles btnUpdat.Click
        Dim sql As String = "Update  Pharm_Safe_Payment set P_ID=@P_ID,P_Date=@P_Date,itemID=@itemID,amount=@amount,amount_abc=@amount_abc,Note=@Note,Supplier_ID = @Supplier_ID,User_ID=@User_ID where ID =@ID"
    ...
    Private Sub btnCancel_Click(sender As Object, e As EventArgs) Handles btnCancel.Click
        Dim sql As String = "Update  Pharm_Safe_Payment set State = @State where ID = @ID"
        Myconn.cmd = New SqlCommand(sql, Myconn.conn)
    ...
    Private Sub drg_CellClick(sender As Object, e As DataGridViewCellEventArgs) Handles drg.CellClick
        Myconn.Filldataset("Select * ,b.itemName from Pharm_Safe_Payment a
                            left join Supplier c on a.Supplier_ID = c.Supplier_ID 
                           left join payment_item b on a.itemID = b.paymentID where a.ID =" & CInt(drg.CurrentRow.Cells(7).Value), "Pharm_Safe_Payment", Me)
        Binding()
