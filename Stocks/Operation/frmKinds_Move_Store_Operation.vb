Public Class frmKinds_Move_Store_Operation
        ...
        Try
            ...
            For i As Integer = 0 To Myconn.cur.Count - 1
                ...
                ElseIf Amount <> 0
                    ...
                    ElseIf B < 0 And C < 0
                        max = B * Myconn.cur.Current("Drug_Price")
                        min = Math.Round((C * E)) * (Myconn.cur.Current("Price") / E)
                        Label4.Text = B & " " & Myconn.cur.Current("Max_Unit_Name") & " و " & Math.Round((C * E)) & " " & Myconn.cur.Current("Min_Unit_Name")
                    ElseIf B = 0 And C < 0
                        max = 0
                        min = Math.Round((C * E)) * (Myconn.cur.Current("Drug_Price") / E)
                ...
                drg.Rows(i).Cells(4).Value = Myconn.cur.Current("Amount_Pur")
                drg.Rows(i).Cells(5).Value = Myconn.cur.Current("Amount_sales")
                drg.Rows(i).Cells(6).Value = Label4.Text
                drg.Rows(i).Cells(7).Value = max + min
                drg.Rows(i).Cells(8).Value = Myconn.cur.Current("GroupName")

                Myconn.cur.Position += 1
            Next
            Myconn.Sum_drg(drg, 7, Label39, Label40)
        Catch ex As Exception
            MsgBox("هناك خطأ في البيانات")
        End Try
    End Sub
    Private Sub frmKinds_Move_Store_Operation_Load(sender As Object, e As EventArgs) Handles Me.Load
        Label5.Left = 0
    ...
    Private Sub frmKinds_Move_Store_Operation_Activated(sender As Object, e As EventArgs) Handles Me.Activated
        StockID = 2
    End Sub
End Class